...
const SkillFoldersList: React.FC = () => {
  const { workspaceId } = useParams<{ workspaceId: string }>();
  const { skillFolders, isLoading, hasError, retry, deleteSkillFolderById } = useSkillFolders(workspaceId);

  const [deleteTarget, setDeleteTarget] = useState<SkillFolder | null>(null);
  ...
  const handleOpenDeleteModal = useCallback((skillFolder: SkillFolder) => {
    setDeleteError(null);
    setDeleteTarget(skillFolder);
  }, []);
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    setDeleteError(null);
    try {
      await deleteSkillFolderById(deleteTarget.id);
      setDeleteTarget(null);
      setToast({ message: `Skill folder '${deleteTarget.name}' deleted.`, type: 'success' });
    } catch (err) {
      setDeleteError(err instanceof Error ? err.message : 'Failed to delete skill folder. Please try again.');
    } finally {
      setIsDeleting(false);
    }
  }, [deleteTarget, workspaceId, deleteSkillFolderById]);
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-text">Skill Folders</h1>
        <Link
          to={newFolderPath}
      ...
      {hasError && (
        <div className="flex items-center justify-between gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span>Failed to load skill folders.</span>
          </div>
          <button
            onClick={retry}
      ...
      {/* Empty state */}
      {!isLoading && !hasError && skillFolders.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-textMuted">
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center">
            <Folder className="w-7 h-7 text-primary opacity-60" />
          </div>
          <div className="text-center">
            <p className="text-base font-medium text-text">No skill folders yet</p>
            <p className="text-sm mt-1">Register folders containing SKILL.md files to extend your agents.</p>
          </div>
          <Link
            to={newFolderPath}
            className="px-4 py-2 bg-primary text-white rounded-lg font-medium hover:bg-primaryHover transition-colors text-sm"
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-md bg-surface border border-border rounded-xl shadow-2xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-text">Delete Skill Folder</h3>
                <p className="text-sm text-textMuted">
                  Are you sure you want to delete <span className="font-medium text-text">"{deleteTarget.name}"</span>? This will also unassign it from any agents using it.
                </p>
              </div>
            </div>
...
export default SkillFoldersList;
