...
export const createAgent = async (workspaceId: string, data: { name: string; role: string; capabilities: string[]; toolActionIds: string[]; mcpSelections?: McpToolSelection[]; customInstructions?: string; projectPrinciples?: string; model?: string | null; subAgentIds?: string[]; skillIds?: string[] }): Promise<Agent> => {
  try {
